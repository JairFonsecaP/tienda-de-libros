...
public class TiendaDeLibros {
     ...
     * 
     * @param pTitulo El titulo del libro que se quiere buscar. pTitulo != null &&
     *                pTitulo != "".
     * @return Si existe un libro con ese título, lo retorna. En caso contrario,
     *         retorna null.
     */
     ...
     * 
     * @param pIsbn El código ISBN del libro que se quiere buscar. pIsbn != null &&
     *              pIsbn != "".
     * @return Si existe un libro con ese ISBN, lo retorna. En caso contrario,
     *         retorna null.
     */
     ...
     * 
     * @param pTitulo       El título del libro que se quiere agregar. pTitulo !=
     *                      null && pTitulo != "".
     * @param pIsbn         El código ISBN del libro que se quiere agregar. pIsbn 
     *                      ull && pIsbn != "".
     * @param pPrecioVenta  El precio de venta del libro que se quiere agregar.
     *                      pPrecioVenta > 0.
     * @param pPrecioCompra El precio de compra del libro que se quiere agregar.
     *                      pPrecioCompra > 0.
     * @param pRutaImagen   La ruta de la imagen del libro. pRutaImagen != null &&
     *                      pRutaImagen != "".
     * @return El nuevo libro registrado en caso de que si se haya podido realizar
     *         la operación, null en caso de que el libro ya exista.
     */
    public Libro registrarLibro(String pTitulo, String pIsbn, double pPrecioVenta, double pPrecioCompra,
            String pRutaImagen) {
    ...
    /**
     * Elimina un libro con el ISBN dado por parámetro. Si la cantidad actual de
     * ejemplares es mayor a cero, no se eliminará el libro. <br>
     * <b>post: </b> El libro fue eliminado del catálogo.
     * 
     * @param pIsbn El ISBN del libro que se quiere eliminar. pIsbn != null && pIsbn
     *              != "".
     * @return Retorna true si se pudo eliminar, false si el libro no existe o si la
     *         cantidad actual de ejemplares es mayor a cero.
     */
     ...
     * <b>post: </b> Se abasteció el libro con el ISBN dado y se disminuyó la
     * cantidad en caja con el precio final del abastecimiento.
     * 
     * @param pIsbn     El Código ISBN del libro que se quiere abastecer. pIsbn!=
     *                  null && pISBN != "".
     * @param pFecha    La fecha en la que se realizó la transacción. pFecha != ""
     *                  && pFecha != null.
     * @param pCantidad La cantidad de ejemplares que se van a abastecer. pCantidad
     *                  >= 0.
    ...
    /**
     * Vende la cantidad de ejemplares del libro con el ISBN dado por parámetro.
     * <br>
     * <b>post: </b> Se vendió el libro con el ISBN dado y se aumentó la cantidad e
     *  ja con el precio final de la venta.
     * 
     * @param pIsbn     El Código ISBN del libro que se quiere vender. pIsbn != 
     *                  pIsbn != "".
     * @param pCantidad La cantidad de ejemplares que se van a vender.
     * @param pFecha    La fecha en la que se realizó la transacción. pFecha != ""
     *                  && pFecha != null.
     * @return Retorna true en caso de que se pueda vender la cantidad de ejemplares
     *         dada por parámetro. False en caso contrario.
     */
    ...
    /**
     * Busca el libro más costoso, es decir el libro con el mayor precio de venta 
     * el catálogo.
     * 
     * @return El libro más costoso. En caso de que el catalogo esté vacío, re or
     *         a l
     */
    ...
    /**
     * Busca el libro más económico en el catálogo de libros. El libros más
     * económico es el libro con el menor precio de venta.
     * 
     * @return El libro menos costoso. En caso de que el catálogo esté vacío,
     *         retorna null.
     */
    public Libro darLibroMasEconomico() {
        ...
        if (catalogo.size() > 0) {
            // Para comenzar el recorrido, se asume que el primer libro es el más 
            // conómico
    ...
    /**
     * Busca el libro más vendido, es decir el libro con más transacciones de tipo
     * VENTA.
     * 
     * @return El libro más vendido. En caso de que el catálogo esté vacío, ret rna
     *         
     * 
     * 
     * 
    ...
    /**
     * Calcula la cantidad de transacciones de abastecimiento del libro con el ISBN
     * dado por parámetro.
     * 
     * @param pIsbn El código ISBN del libro que se quiere buscar. pIsbn != null &&
     *              pIsbn != "".
     * @return La cantidad de transacciones de abastecimiento. En caso de que no
     *         encuentre el libro o no tenga transacciones, retorna cero.
     */
    ...
    public String metodo1(int[] lim_inf, int[] lim_sup, double[] descuentos) {
        for (int i = 0; i < darCatalogo().size(); i++) {
            for (int j = 0; j < descuentos.length; j++) {
                if (catalogo.get(i).darPrecioVenta() >= lim_inf[j] && catalogo.get(i).darPrecioVenta() < lim_sup[j]) {
                    double precioModificado = catalogo.get(i).darPrecioVenta()
                            - (catalogo.get(i).darPrecioVenta() * descuentos[j]);
                    catalogo.get(i).actualizarPrecio(precioModificado);
                }
            }
        }
    ...
    public Transaccion metodo2() {
        ...
        int mayorTransaccion = 0;
        Transaccion mayorVenta = null;
        for (Libro libroActual : catalogo) 
        {
            for (Transaccion transaccionActual : libroActual.darTransacciones()) 
            {
                if (transaccionActual.darTipo().equals(Transaccion.Tipo.VENTA) 
                		&& transaccionActual.darCantidad() > mayorTransaccion) 
                {
                    mayorVenta = transaccionActual;
                    mayorTransaccion = transaccionActual.darCantidad();
                }
            }
